...
上一节已经从网页界面角度说明了交互参数、步进视图、目标态曲线、量子电路视图以及带噪声对照等页面内容。本节不再重复介绍界面布局，而是从数值结果角度检查当前实现是否符合Grover算法的基本演化规律。结果分析的重点放在理想statevector下的概率幅变化、目标态概率提升以及过迭代现象；带噪声模拟只作为附加对照，用于说明测量噪声可能带来的偏离，并不作为本项目的核心评价指标。

\subsection{迭代次数口径说明}

为避免混淆，本文将Grover迭代次数记为\(r\)。一次完整Grover迭代包含一次Oracle相位翻转和一次Diffusion均值倒置；Hadamard初始化不计入Grover迭代次数，因此初始化后的状态对应\(r=0\)。
